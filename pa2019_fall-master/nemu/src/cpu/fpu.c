...
inline uint32_t internal_normalize(uint32_t sign, int32_t exp, uint64_t sig_grs)
{
	...
	if ((sig_grs >> (23 + 3)) > 1 || exp < 0)
	{
		...
		//printf("sdjlasjdlkasjdklsa  1\n");
		uint32_t sticky_flag=0;
		while ((((sig_grs >> (23 + 3)) > 1) && exp < 0xff) // condition 1
			   ||										   // or
			   (sig_grs > 0x04 && exp < 0)				   // condition 2
			   )
		{
			sticky_flag=sticky_flag|(sig_grs&0x1);
			sig_grs=sig_grs>>1;
			exp++;
			sig_grs=sig_grs|sticky_flag;
			//printf("shift right\n");
		}
		if (exp >= 0xff)
		{
	...
	{
		//printf("sdjlasjdlkasjdklsa  3\n");
		// two denormals result in a normal
		exp++;
	}
	//uint16_t hhh=sig_grs;
	//printf("hhh %x\n",hhh);
	if (!overflow)
		{
